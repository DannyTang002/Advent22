...
public class Advent12part2 {
    private static int[][] grid;
    private static char[][] gridchar;
    private static List<Point> length = new ArrayList<>();
    private static List<Integer> minLengths = new ArrayList<>();
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < file.size(); i++) {
            ...
            for (int j = 0; j < array.length; j++) {
                ...
                if (array[j] == 'S' || array[j] == 'a') {
                    y = i;
                    x = j;
                    length.add(new Point(x, y));
        ...
        for (Point p : length) {
            minLengths.add(findPath((int) p.getX(), (int) p.getY(), file.size(), file.get(0).length()));
        }
        for (int s : minLengths) {
            System.out.println(s);
        }

        System.out.println(minLengths.stream().mapToInt(p -> p).filter(p -> p >= 300).min().orElse(0));
    }
